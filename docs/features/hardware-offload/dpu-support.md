...
The hosts and their corresponding DPUs are added as worker nodes in the OVN-Kubernetes cluster. The steps required to configure SR-IOV and enable multihoming on the DPU-Host is provided below.
...
## DPU-host Worker Node Multus CNI configuration
...
The field OVNKUBE_NODE_MGMT_PORT_NETDEV needs to be set in the ovnkube-node-dpu-host.yaml indicating the VF device to be used for management port. For example, "enp2s0f0v0"
...
In addition, a node label is required to indicate to the nodes running ovnkube-master that the DPU worker node is managing its own network, "noHostSubnetLabel". By
default in the ovnkube-master deployments this is set to
...
Some basic CRDs are already provided under helm/ovn-kubernetes/crds. If additional CRDs are required, they should be placed in that location. For example, to support multihoming and pod isolations, the following will be required. 
...
k8s.cni.cncf.io-multi-networkpolicies - https://github.com/k8snetworkplumbingwg/multi-networkpolicy/blob/master/scheme.yml
```
...
#Helm Installation
Install OVN-Kubernetes CNI using helm with https://github.com/ovn-kubernetes/ovn-kubernetes/blob/master/helm/ovn-kubernetes/values-no-ic.yaml by modifying the following settings in the default values-no-ic.yaml to use DPU-Hosts with basic features.
