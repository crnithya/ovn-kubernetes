...
func convertToFloat64(intfVal *int) float64 {
	var value float64
	if intfVal != nil {
		value = float64(*intfVal)
	} else {
		value = 0
	}
	return value
...
func getOvsVersionInfo(ovsDBClient libovsdbclient.Client) {
	openVswitchTable, err := ops.GetOVSOpenvSwitchTable(ovsDBClient)
	if err != nil {
		klog.Errorf("Failed to get ovsdb openvswitch table :(%v)", err)
		return
	}
	if openVswitchTable.OVSVersion != nil {
		ovsVersion = *openVswitchTable.OVSVersion
	} else {
...
func updateOvsBridgeMetrics(ovsDBClient libovsdbclient.Client, ovsOfctl ovsClient) error {
	bridgeList, err := ops.ListOVSBridges(ovsDBClient)
	if err != nil {
...
// updateOvsInterfaceMetrics updates the ovs interface metrics obtained from vswitchd DB
func updateOvsInterfaceMetrics(ovsDBClient libovsdbclient.Client) error {
	interfaceList, err := ops.ListOVSInterfaces(ovsDBClient)
	if err != nil {
...
// through ovsdb-client from Open_vSwitch table updates
func setOvsHwOffloadMetrics(ovsDBClient libovsdbclient.Client) (err error) {
	openVswitchRow, err := ops.GetOVSOpenvSwitchTable(ovsDBClient)
	if err != nil {
